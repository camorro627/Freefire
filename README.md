...
بنية **Master-Worker** بتنفيذ متوازٍ (`asyncio + aiohttp`)، قاعدة بيانات
محلية (`sqlite3`)، ومدير بروكسيات ذكي يجلب ويحلل ويستبدل البروكسيات تلقائياً
من مصادر مجانية — مصممة للتحكم بـ **100+ حساب**.

> **نطاق الاستخدام:** الأداة تتعامل مع الحسابات المسجلة محلياً في قاعدة
> بياناتك فقط (استيراد CSV / add_account / check_player / mass_status).
> لا تستهدف حسابات طرف ثالث. نقطة النهاية `https://garena.mock` وهمية —
> استبدلها فقط بنقطة نهاية تملكها أو فُوِّضت باختبارها.

## البنية

```
freefire_control/
├── config.py              # الثوابت + إعدادات الـ 100 حساب والبروكسيات
...
| `proxy_status` | حالة المخزون + تحليل السلوك (متوسط زمن/نجاح/مريبة) |
